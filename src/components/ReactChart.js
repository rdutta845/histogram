...
class Bar extends React.Component {
  ...
  class YAxis extends React.Component {
    ...
    render() {
      ...
      let columnLables = []
      ticks.forEach((tick, index) => {
        columnLables.push(<text style={ textStyle } y={tick + 6} x={this.props.y - 6}  key={index}>{percentage(this.props.labels[index])}</text>)
      })
  ...
  class XAxis extends React.Component {
    ...
    render() {
      ...
      let columnLables = []
      ticks.forEach((tick, index) => {
        columnLables.push(<text style={{fill: "steelblue"}} x={tick + 5} y={this.props.x + 20} key={index}>{this.props.labels[index]}</text>)
      })
